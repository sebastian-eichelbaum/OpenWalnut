...
int bscs_connect(char* hostname) {
	...
	// IPv4 only 	
   	...
   	if(remoteHost==NULL) return(BSCS_UNKNOWN_HOST);

   	sain.sin_family = remoteHost->h_addrtype;
   	memcpy((char *) &sain.sin_addr.s_addr, remoteHost->h_addr_list[0],remoteHost->h_length);
   	...
   	/* connect to server */
   	rc = connect(sd, (struct sockaddr *) &sain, sizeof(sain));
...
int savelink(const char* filename) {
	...
	logfile = (char*)malloc(strlen(logpath)+strlen(fn)+6+strlen(tmp));
...
if (VERBOSE_LEVEL>8) fprintf(stdout,"write evt: len=%i\n",len); 
	...
	msg.LEN   = b_endian_u32(len);
	s1 = send(sd, TC &msg, 8, 0);	
	s2 = send(sd, TC hdr->AS.rawEventData, len, 0);	
...
/****************************************************************************************
	REQUEST DATA
 ****************************************************************************************/
size_t bscs_requ_dat(int sd, size_t start, size_t length, HDRTYPE *hdr) {
/*
