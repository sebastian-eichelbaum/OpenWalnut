/*
    ...
    $Id: biosig.c 2116 2009-06-05 11:53:36Z schloegl $
...
#ifdef _MSC_VER
#define NaN_AR 0.0//std::numeric_limits<double>::quiet_NaN()
    const double scale[] =
    { 1e+0, 1e+1, 1e+2, 1e+3, 1e+6, 1e+9,  1e+12, 1e+15,	//  0..7 
    1e+18,1e+21,1e+24,NaN_AR,  NaN_AR,  NaN_AR,   NaN_AR,   NaN_AR, 	//  8..15
    1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1e-12, 1e-15, 1e-18, 	// 16..23
    1e-21,1e-24,NaN_AR,  NaN_AR,  NaN_AR,  NaN_AR,   NaN_AR,   NaN_AR,	// 24..31
...
char* PhysDim(uint16_t PhysDimCode, char *PhysDimC)
{	
#ifdef _MSC_VER
    int k;
#else
...
#endif
	strcpy(PhysDimC,PhysDimFactor[PhysDimCode & 0x001F]);

	PhysDimCode &= ~0x001f;
#ifdef HARDCODED_PHYSDIMTABLE 
	for (k=0; _physdim[k].idx<0xffff; k++)
	if (PhysDimCode == _physdim[k].idx) {
		strncat(PhysDimC, _physdim[k].PhysDimDesc, MAX_LENGTH_PHYSDIM);
#else
	for (k=0; Global.PhysDimCode[k]<0xffff; k++)
	if (PhysDimCode == Global.PhysDimCode[k]) {
		strncat(PhysDimC, Global.PhysDim[k], MAX_LENGTH_PHYSDIM);
#endif
		PhysDimC[MAX_LENGTH_PHYSDIM]=0;
		break;
	}
	return(PhysDimC);
}
...
gdf_time tm_time2gdf_time(struct tm *t){
	...
	*/
	...
	// Add number of days to the start of the current year. Correct
  	...
  	D += 365*Y + floor ((double)Y/4) - floor ((double)Y/100) + floor ((double)Y/400);
...
struct tm *gdf_time2tm_time(gdf_time t) {
	...
	*/
	...
	s = ldexp((double) (t & 0x00000000ffffffff),-32)*86400 + 1; // seconds of the day 
	// s += timezone;	
...
#if __sparc__ || __APPLE__ || __MINGW32__ || _MSC_VER
	fpos_t p = *pos;
#else
	fpos_t p;
	p.__pos = *pos; 
...
void convert2to4_eventtable(HDRTYPE *hdr) {
    ...
    if (hdr->EVENT.DUR == NULL) 
        hdr->EVENT.DUR = (uint32_t*) calloc(N,sizeof(*hdr->EVENT.DUR));
    if (hdr->EVENT.CHN == NULL)
        hdr->EVENT.CHN = (uint16_t*) calloc(N,sizeof(*hdr->EVENT.CHN));
#else
	if (hdr->EVENT.DUR == NULL) 
...
#ifdef _MSC_VER
    hdr->EVENT.TYP = (uint16_t*) realloc(hdr->EVENT.TYP,2*N*sizeof(*hdr->EVENT.TYP));
    hdr->EVENT.POS = (uint32_t*) realloc(hdr->EVENT.POS,2*N*sizeof(*hdr->EVENT.POS));
#else
...
void LoadGlobalEventCodeTable() 
{
	...
	N = 0; 
	line = strtok(Global.EventCodesTextBuffer,"\x0a\x0d");
...
#ifdef _MSC_VER
        hdr->EVENT.CodeDesc = (char**) realloc(hdr->EVENT.CodeDesc,257*sizeof(*hdr->EVENT.CodeDesc));
#else
 ...
 */
	...
	for (k=0;k<hdr->NS;k++)	{
		...
		BitsPerBlock   += nbits; 
	      	...
	      	hc->LowPass   = 70.0;
	      	hc->Notch     = 50;
            hc->Impedance = 100000000000000.0;
//            hc->Impedance = INF;
	      	hc->XYZ[0] 	  = 0.0;
	      	hc->XYZ[1] 	  = 0.0;
	      	hc->XYZ[2] 	  = 0.0;
    ...
    size_t bpb8 = 0; 
		...
		for (k=0; k<hdr->NS; k++)	{
			...
			if (hdr->VERSION < 1.90) {
				...
				PreFilt  = (char*)(Header2+ 68*k + 136*hdr->NS);
				hc->LowPass  = 0.0;
				hc->HighPass = 0.0;
				hc->Notch    = 0.0;
...
size_t hdrEVT2rawEVT(HDRTYPE *hdr) { 
	...
	buf1=hdr->AS.rawEventData+8;
	buf2=hdr->AS.rawEventData+8+hdr->EVENT.N*4;
	for (k32u=0; k32u<hdr->EVENT.N; k32u++) {
...
//			int sze = (buf[0]>1) ? 12 : 6;
            ...
            buf1 = hdr->AS.rawEventData+8; 
                buf2 = hdr->AS.rawEventData+8+4*hdr->EVENT.N; 
			for (k=0; k < hdr->EVENT.N; k++) {
...
/**                     SOPEN                                              **/
/****************************************************************************/
HDRTYPE* biosig_sopen(const char* FileName, const char* MODE, HDRTYPE* hdr)
/*
	MODE="r" 
 ...
 */
{
	...
	const float	CNT_SETTINGS_HIGHPASS[] = {0.0, 0, .05, .1, .15, .3, 1, 5, 10, 30, 100, 150, 300};
...
if (!strncmp(MODE,"r",1))	
{
	...
	FileExt  = hdr->FileName+ext;
	FileName = hdr->FileName+name;

	/* AINF */
	if (!strcmp(FileExt, "ainf")) {
        char* AINF_RAW_FILENAME;
        FILE* fid1;
		if (VERBOSE_LEVEL>8) fprintf(stdout,"getfiletype ainf1 %s %i\n",hdr->FileName,ext); 
		AINF_RAW_FILENAME = (char*)calloc(strlen(hdr->FileName)+5,sizeof(char));
		strncpy(AINF_RAW_FILENAME, hdr->FileName,ext);
		strcpy(AINF_RAW_FILENAME+ext, "raw");
		fid1=fopen(AINF_RAW_FILENAME,"rb");
		if (fid1) {
			fclose(fid1);
			hdr->TYPE = AINF; 
		}	
		...
		fid1=fopen(AINF_RAW_FILENAME,"r");
		if (fid1) {
			fclose(fid1);
			hdr->TYPE = AINF; 
		}	
...
//		fprintf(stdout,"[201] FMT=%s Ver=%4.2f\n",GetFileTypeString(hdr->TYPE),hdr->VERSION);
	...
	if (hdr->TYPE == GDF) {
		...
		else if (FileBuf.st_size > hdr->HeadLen + hdr->AS.bpb*hdr->NRec + 8) 
		{
			...
			c = ifread(hdr->AS.rawEventData, sizeof(uint8_t), 8, hdr);
    		buf = hdr->AS.rawEventData;
...
//			if (ptr_str != NULL) { 
		...
		if (hdr->NS==0) return(hdr); 
	    	...
	    	Header2 = (char*)hdr->AS.Header+256; 
	    	count  += ifread(hdr->AS.Header+count, 1, hdr->HeadLen-count, hdr);
		...
		for (k=0, hdr->SPR = 1; k<hdr->NS; k++)	{
            ...
            PreFilt = (Header2+ 80*k + 136*hdr->NS);
			PreFilt[79] = 0;
...
//					fprintf(stdout,"%i: %s\n",c,buf); 
		...
		hdr->CHANNEL = (CHANNEL_TYPE*) calloc(hdr->NS,sizeof(CHANNEL_TYPE));
            ACQ_NoSamples = (uint32_t*) calloc(hdr->NS,sizeof(uint32_t));
		...
		for (k = 0; k < hdr->NS; k++)	{
			...
			ACQ_NoSamples[k] = leu32p(Header2+88);
                tmp64 = leu32p(Header2+88) * hc->SPR; 
...
*/
		...
		ifclose(hdr);
		filename = hdr->FileName; // keep input file name 
		tmpfile = (char*)calloc(strlen(hdr->FileName)+5,1);		
		strcpy(tmpfile,hdr->FileName);
		ext = strrchr(tmpfile,'.')+1; 
...
if (VERBOSE_LEVEL>8) fprintf(stdout,"BIN <%s>=<%s> \n",line,val); 
			...
			if (status==1) {
				...
				if (!strncmp(line,"0x",2)) {
					...
					if (i>0xffff)
						...
						hdr->EVENT.TYP[hdr->EVENT.N] = (uint16_t)i;	

					double d;	
					val = strchr(val,'\t')+1;
					sscanf(val,"%lf",&d);
					hdr->EVENT.POS[hdr->EVENT.N] = (uint32_t)round(d*hdr->EVENT.SampleRate);
					
					val = strchr(val,'\t')+1;
					if (val[0]!='\t') {
						sscanf(val,"%lf",&d);
						hdr->EVENT.DUR[hdr->EVENT.N] = (uint32_t)round(d*hdr->EVENT.SampleRate);
					}
					else  
		...
		if (ptr==NULL) 
			...
			/* read whole header */
			hdr->HeadLen = (uint32_t) strtod(ptr+10,&ptr);
		...
		if (ptr==NULL) 
			...
			hdr->NS = (uint16_t) strtod(ptr+9,&ptr); 
...
#undef gain
		...
		strcpy(tmpfile,hdr->FileName);
		hdr->FileName = tmpfile;
		const char* ext = strrchr(hdr->FileName,'.')+1; 
...
//                    strcpy(ext,strrchr(t,'.')+1);		
					...
					HDRTYPE *hdr2 = biosig_sopen(mrkfile,"r",NULL);
...
*/
		...
		for (k=0; k<n1; k++) {
			...
			hdr->EVENT.TYP = (uint16_t*) realloc(hdr->EVENT.TYP,(hdr->EVENT.N+n2)*sizeof(*hdr->EVENT.TYP));
			hdr->EVENT.POS = (uint32_t*) realloc(hdr->EVENT.POS,(hdr->EVENT.N+n2)*sizeof(*hdr->EVENT.POS));
			hdr->EVENT.CHN = (uint16_t*) realloc(hdr->EVENT.CHN,(hdr->EVENT.N+n2)*sizeof(*hdr->EVENT.CHN));
			hdr->EVENT.DUR = (uint32_t*) realloc(hdr->EVENT.DUR,(hdr->EVENT.N+n2)*sizeof(*hdr->EVENT.DUR));
...
//			if (n2>1) fprintf(stdout,"EEG1100: more than 1 segment [%i,%i] not supported.\n",n1,n2);
			for (k2=0; k2<n2; k2++) {
				...
				uint16_t NS = h3[38];
				double SampleRate = leu16p(h3+26) & 0x3fff;
		...
		if (c != NULL) {
			...
			if (fid != NULL) { 
				...
				for (k=0; k<LOG[145]; k++) {
					...
					hdr->EVENT.TYP = (uint16_t*) realloc(hdr->EVENT.TYP,(hdr->EVENT.N+N)*sizeof(*hdr->EVENT.TYP));
					hdr->EVENT.POS = (uint32_t*) realloc(hdr->EVENT.POS,(hdr->EVENT.N+N)*sizeof(*hdr->EVENT.POS));
					hdr->EVENT.CHN = (uint16_t*) realloc(hdr->EVENT.CHN,(hdr->EVENT.N+N)*sizeof(*hdr->EVENT.CHN));
					hdr->EVENT.DUR = (uint32_t*) realloc(hdr->EVENT.DUR,(hdr->EVENT.N+N)*sizeof(*hdr->EVENT.DUR));
					size_t k1;
					for (k1=0; k1<N; k1++) {
						...
						if (t0 >= hdr->T0) 
						{
							hdr->EVENT.POS[hdr->EVENT.N] = (uint32_t)(ldexp((double)t0 - hdr->T0,-32)*24*3600*hdr->SampleRate);
		...
		for (k=0; k<hdr->NS; k++) {
			...
			hc->Cal	= PhysMax/ldexp((double)1,Bits);
			hc->Off	= 0;
			hc->SPR	= hdr->SPR;
			hc->bi	= k*hdr->SPR*GDFTYP_BITS[gdftyp]>>3;
			...
			if (Bits && PhysMax) {
				hc->PhysMax = PhysMax;
				hc->PhysMin = -PhysMax;
				hc->DigMax  = ldexp((double)1,Bits);
				hc->DigMin  = ldexp((double)-1,Bits);
			} 
			else {	
/*			hc->PhysMax = PhysMax;
			hc->PhysMin = -PhysMax;
	 	...
	 	for (k=0; k<hdr->NS; k++) {
			...
			CHANNEL_TYPE *hc = hdr->CHANNEL+k;
	 		...
	 		if (gdftyp<16)
	 			...
	 			hc->DigMax =  100000000.0; 
		 		hc->DigMin = -100000000.0; 
	 		}
	 		hc->PhysMax = hc->DigMax * hc->Cal + hc->Off; 
	 		hc->PhysMin = hc->DigMin * hc->Cal + hc->Off; 
	    		hc->OnOff   = 1;
...
//	 	hdr2ascii(hdr,stdout,4);
		...
		hdr->NS = (uint16_t)strtod(ptr+1,&ptr);		// number of channels 
...
#undef line
		hdr->SPR = (uint32_t)round(hdr->SampleRate/minFs);
		for (k=0,hdr->AS.bpb=0; k<hdr->NS; k++) {
			hdr->CHANNEL[k].SPR = (uint32_t)round(fs[k]/minFs);
...
*/	
		...
		if ((filetype>1) && (filetype<6))
			Cal = ldexp((double)1,-GDFTYP_BITS[gdftyp]);
		...
		hdr->FLAG.OVERFLOWDETECTION = 0; 	// automated overflow and saturation detection not supported
		hdr->CHANNEL = (CHANNEL_TYPE*)calloc(hdr->NS, sizeof(CHANNEL_TYPE));
		double digmax = ldexp((double)1,GDFTYP_BITS[gdftyp]);
		for (k=0,hdr->AS.bpb=0; k < hdr->NS; k++) {
			CHANNEL_TYPE* hc = hdr->CHANNEL+k;
			hc->OnOff    = 1;
			hc->GDFTYP   = gdftyp;
			...
			if (!fid) {
				...
				if (0) {
					...
					if (fgets(line,sz,fid)) {
						...
						int k2;
						for (k2=0;k2<hdr->NS;k2++) 
							*(double*)(hdr->AS.rawdata+(k*hdr->NS+k2)*sizeof(double)) = 0.0;
					}
				}	
				free(line);
				fclose(fid);
		...
		while (1) {
			...
			if (!strncmp((char*)tag,"fmt ",4)) {
				...
				if (format==1) {
					...
					Cal 	= ldexp((double)1,-8*(bits/8 + ((bits%8) > 0)));
					if 	(bits <= 8) {
						gdftyp = 2;
						Off = 0.5;
					}	 
...
//	    		fprintf(fid,"Area     \t= %f\n",hdr->CHANNEL[k].Area);
		...
		uint32_t us = (hdr->T0*24*3600 - floor((double)hdr->T0*24*3600))*1e6;
...
/**	SREAD : segment-based                                              **/
/****************************************************************************/
size_t biosig_sread(biosig_data_type* data, size_t start, size_t length, HDRTYPE* hdr) {
/* 
 *	Reads LENGTH blocks with HDR.AS.bpb BYTES each 
 * 	(and HDR.SPR samples). 
 *	Rawdata is available in hdr->AS.rawdata.
 ...
 */
	...
	CHANNEL_TYPE		*CHptr;
	int32_t			int32_value;
	biosig_data_type 	sample_value=0.0; 
...
*/
		...
		if ((hdr->FLAG.OVERFLOWDETECTION) && ((sample_value <= CHptr->DigMin) || (sample_value >= CHptr->DigMax)))
			sample_value = 0.0; 	// missing value 
				...
				if (CHptr->SPR==0) {	
					...
					if (hdr->FLAG.ROW_BASED_CHANNELS) {
						for (k5 = 0; k5 < hdr->SPR*count; k5++)
							data[k2 + k5*NS] = 0.0;		// row-based channels 
					} else {
						for (k5 = 0; k5 < hdr->SPR*count; k5++)
							data[k2*count*hdr->SPR + k5] = 0.0; 	// column-based channels 
					}
				}
				k2++;
			}
...
#define MIN_UINT64 ((uint64_t)0)
	...
	// memory allocation for SCP is done in SOPEN_SCP_WRITE Section 6	
		ptr = (uint8_t *)realloc(hdr->AS.rawdata, (hdr->NRec*bpb8>>3)+1);
		if (ptr==NULL) {
			B4C_ERRNUM = B4C_INSUFFICIENT_MEMORY;
...
/****************************************************************************/
HDRTYPE* biosig_sload(char* FileName, size_t ChanList[], biosig_data_type** DATA)
{
	size_t k,NS;
	int status;
	HDRTYPE* hdr; 
	hdr = biosig_sopen(FileName,"r",NULL); 
	if ((status=serror())) {
		destructHDR(hdr);
		return(NULL);
	} 
	...
	*DATA = (biosig_data_type*)realloc(*DATA,hdr->SPR*hdr->NRec*NS*sizeof(biosig_data_type));
	biosig_sread(*DATA,0,hdr->NRec,hdr); 	
	biosig_sclose(hdr);
	return(hdr); 
}
...
//		fprintf(fid,"\nPID:\t|%s|\nPatient:\n",hdr->AS.PID);
		...
		if (hdr->Patient.Birthday) {
			...
			fprintf(fid,"\tAge             : %4.1f years\n\tBirthday        : (%.6f) %s ",age,ldexp((double)hdr->Patient.Birthday,-32),asctime(T0));
		}
		else
			fprintf(fid,"\tAge             : ----\n\tBirthday        : unknown\n");
		...
		T0 = gdf_time2tm_time(hdr->T0);
		strftime(tmp, 59, "%x %X %Z", T0);
		fprintf(fid,"\tStartOfRecording: (%.6f) %s\n",ldexp((double)hdr->T0,-32),asctime(T0));
		if (hdr->AS.bci2000 != NULL) {
