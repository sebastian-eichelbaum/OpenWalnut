		...
		if (fid != NULL) {
			...
			while (t[0]) {
				...
				if (n+1 >= N) {
					const size_t sz = 100;
#ifdef _MSC_VER
                    hdr->EVENT.TYP = (BOOST_TYPEOF(hdr->EVENT.TYP)) realloc(hdr->EVENT.TYP,(N+sz)*sizeof(*hdr->EVENT.TYP));
                    hdr->EVENT.POS = (BOOST_TYPEOF(hdr->EVENT.POS)) realloc(hdr->EVENT.POS,(N+sz)*sizeof(*hdr->EVENT.POS));
#else
