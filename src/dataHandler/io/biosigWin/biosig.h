...
#ifdef _MSC_VER
#define ATT_ALI __declspec(align(8))
#else
#define ATT_ALI __attribute__ ((aligned (8)))	/* Matlab v7.3+ requires 8 byte alignment*/
#endif

#ifdef _MSC_VER
typedef ATT_ALI struct {
    ATT_ALI double 		PhysMin;	/* physical minimum */
    ATT_ALI double 		PhysMax;	/* physical maximum */
    ATT_ALI double 		DigMin 	;	/* digital minimum */
    ATT_ALI double	 	DigMax;	/* digital maximum */
    ATT_ALI double		Cal;	/* gain factor */
    ATT_ALI double		Off;	/* bias */

    ATT_ALI char		OnOff;
    ATT_ALI char		Label[MAX_LENGTH_LABEL+1]; 	/* Label of channel */
    ATT_ALI uint16_t	LeadIdCode;	/* Lead identification code */
    ATT_ALI char 		Transducer[MAX_LENGTH_TRANSDUCER+1];	/* transducer e.g. EEG: Ag-AgCl electrodes */
    ATT_ALI char 		PhysDim[MAX_LENGTH_PHYSDIM+1]  /*	__attribute__ ((deprecated))*/;	/* physical dimension */
    /*PhysDim is now depreciated - use function PhysDim(PhysDimCode,PhysDimText) instead */
    ATT_ALI uint16_t	PhysDimCode ;	/* code for physical dimension */
    /* char* 	PreFilt;	// pre-filtering */

    ATT_ALI float 		LowPass		;	/* lowpass filter */
    ATT_ALI float 		HighPass	;	/* high pass */
    ATT_ALI float 		Notch		;	/* notch filter */
    ATT_ALI float 		XYZ[3]		;	/* sensor position */
    ...
    ATT_ALI float 		Impedance	;   	/* in Ohm */

    ATT_ALI uint16_t 	GDFTYP 		;	/* data type */
    ATT_ALI uint32_t 	SPR 		;	/* samples per record (block) */
#ifndef NO_BI
    ATT_ALI uint32_t	bi 		;	/* start byte (byte index) of channel within data block */
    ATT_ALI uint32_t	bi8 		;	/* start bit  (bit index) of channel within data block */
#endif
    ATT_ALI uint8_t*	bufptr		;	/* pointer to buffer: NRec<=1 and bi,bi8 not used */
} CHANNEL_TYPE;
...
typedef struct {
    ...
    ATT_ALI float 		VERSION ;	/* GDF version number */
    ATT_ALI const char* 	FileName;

    ATT_ALI struct {
        ATT_ALI size_t 			size[2] ; /* size {rows, columns} of data block	 */
        ATT_ALI biosig_data_type* 	block ; 	 /* data block */
    } data ;

    ATT_ALI uint32_t 	HeadLen ;	/* length of header in bytes */
    ATT_ALI uint16_t 	NS 	;	/* number of channels */
    ATT_ALI uint32_t 	SPR 	;	/* samples per block (when different sampling rates are used, this is the LCM(CHANNEL[..].SPR) */
    ATT_ALI nrec_t  	NRec 	;	/* number of records/blocks -1 indicates length is unknown. */
    __declspec(deprecated) uint32_t 	Dur[2] 	/*__attribute__ ((deprecated))*/;	/* Duration of each block in seconds expressed in the fraction Dur[0]/Dur[1]  */
    ATT_ALI double 		SampleRate ;	/* Sampling rate */
    ATT_ALI uint8_t 	IPaddr[16] ; 	/* IP address of recording device (if applicable) */
    ATT_ALI uint32_t  	LOC[4] 	;	/* location of recording according to RFC1876 */
    ATT_ALI gdf_time 	T0 	; 	/* starttime of recording */
    ATT_ALI int16_t 	tzmin 	;	/* time zone (minutes of difference to UTC */
    ...
    /* EVENTTABLE */
    ATT_ALI struct {
        ATT_ALI double  	SampleRate ;	/* for converting POS and DUR into seconds  */
        ATT_ALI uint32_t  	N ;	/* number of events */
        ATT_ALI uint16_t 	*TYP ;	/* defined at http://cvs.sourceforge.net/viewcvs.py/biosig/biosig/t200/eventcodes.txt?view=markup */
        ATT_ALI uint32_t 	*POS ;	/* starting position [in samples] */
        ATT_ALI uint32_t 	*DUR ;	/* duration [in samples] */
        ATT_ALI uint16_t 	*CHN ;	/* channel number; 0: all channels  */
        ATT_ALI char		**CodeDesc ;	/* describtion of "free text"/"user specific" events (encoded with TYP=0..255 */
        ATT_ALI uint16_t	LenCodeDesc ;	/* length of CodeDesc Table */
    } EVENT ;

    ATT_ALI struct {	/* flags */
...
size_t	biosig_sread(biosig_data_type* DATA, size_t START, size_t LEN, HDRTYPE* hdr);
...
size_t  biosig_swrite(const biosig_data_type *DATA, size_t NELEM, HDRTYPE* hdr);
