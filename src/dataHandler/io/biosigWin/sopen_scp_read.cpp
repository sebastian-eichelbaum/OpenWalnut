...
*/	
	...
	uint8_t		tag;
#ifdef _MSC_VER
    float LowPass = std::numeric_limits<float>::infinity();
#else
    float LowPass =  LowPass=1.0/0.0;
#endif
	float 		HighPass=0, Notch=-1; 	// filter settings
...
//					t1.tm_gmtoff  =  0; 
					...
					if (bitmap==0)
#ifdef _MSC_VER
                        Notch = std::numeric_limits<float>::quiet_NaN();
#else
						Notch = NaN;	// undefined 
#endif
		...
		/**** SECTION 2 ****/
			...
			if (NHT==19999) {
				...
				for (k2=0; k2<NHT; k2++) {
					...
					curSectPos += 2;
#ifdef _MSC_VER
                    Huffman[k2].Table = (BOOST_TYPEOF(Huffman[k2].Table))malloc(Huffman[k2].NCT * sizeof(*Huffman[k2].Table));
#else
...
#ifdef _MSC_VER
            en1064.Section3.lead = (BOOST_TYPEOF(en1064.Section3.lead))malloc(hdr->NS*sizeof(*en1064.Section3.lead));
#else
...
#ifdef _MSC_VER
            en1064.Section4.beat	= (BOOST_TYPEOF(en1064.Section4.beat))malloc(en1064.Section4.N*sizeof(*en1064.Section4.beat));
#else
...
#ifdef _MSC_VER
            en1064.Section5.inlen	= (BOOST_TYPEOF(en1064.Section5.inlen))malloc(hdr->NS*sizeof(*en1064.Section5.inlen));
#else
...
#ifdef _MSC_VER
            en1064.Section6.inlen	= (BOOST_TYPEOF(en1064.Section6.inlen))malloc(hdr->NS*sizeof(*en1064.Section6.inlen));
#else
...
#ifdef _MSC_VER
            BOOST_TYPEOF(hdr->SPR) SPR  = ( en1064.FLAG.BIMODAL ? en1064.Section4.SPR : hdr->SPR);  
#else
