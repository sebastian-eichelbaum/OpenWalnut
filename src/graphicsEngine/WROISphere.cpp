...
WROISphere::WROISphere( WPosition_2 position, float radius ) :
    WROI(),
    ...
    m_lockPoint( WVector3d_2( 0.0, 0.0, 0.0 ) ),
    m_lockVector( WVector3d_2( 1.0, 1.0, 1.0 ) ),
...
WPosition_2 WROISphere::getPosition() const
{
    return m_position;
}

void WROISphere::setPosition( WPosition_2 position )
{
    m_position = position;
...
{
    m_position = WPosition_2( x, y, z );
    m_lockPoint = WPosition_2( x, y, z );
    m_originalPosition = WPosition_2( x, y, z );
    m_dirty->set( true );
}
...
void WROISphere::moveSphere( WVector3d_2 offset )
{
    m_position += offset;
...
void  WROISphere::setLockVector( WVector3d_2 vector )
{
    m_lockVector = vector;
    m_lockPoint = m_position;
}
