...
WGEGraphicsWindowMac::WGEGraphicsWindowMac( int x,
                                      int y,
                                      int width,
                                      int height )
...
WGEGraphicsWindowMac::~WGEGraphicsWindowMac()
{
    // cleanup
}

void WGEGraphicsWindowMac::resize( int width, int height )
{
    m_GraphicsWindow->getEventQueue()->windowResize( 0, 0, width, height );
    m_GraphicsWindow->resized( 0, 0, width, height );
}

void WGEGraphicsWindowMac::close()
{
    m_GraphicsWindow->getEventQueue()->closeWindow();
}

void WGEGraphicsWindowMac::keyEvent( KeyEvents eventType, int key )
{
    switch( eventType )
    {
        case KEYPRESS:
...
void WGEGraphicsWindowMac::mouseEvent( MouseEvents eventType, int x, int y, int button )
{
    switch( eventType )
    {
        case MOUSEPRESS:
