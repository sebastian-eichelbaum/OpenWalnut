...
public:
    ...
    void testMeanFunctionOutsideOf_R_Environment( void )
    {
        ...
        TS_ASSERT_DELTA( p.mean( WPosition_2( -( p.m_R + wlimits::DBL_EPS ), 0.0, 0.0 ) ), 0.0, wlimits::DBL_EPS );
    }
    ...
    {
        WGaussProcess p( m_tractID, m_tracts, m_emptyDTIDataSet );
        TS_ASSERT( std::abs( p.mean( WPosition_2( -p.m_R + 1.0e-7, 0.0, 0.0 ) ) ) > wlimits::DBL_EPS );
    }
    ...
    {
        WGaussProcess p( m_tractID, m_tracts, m_emptyDTIDataSet );
        TS_ASSERT( std::abs( p.mean( WPosition_2( -p.m_R + 1.0e-8, 0.0, 0.0 ) ) ) >
                             p.mean( WPosition_2( -p.m_R + 0.5e-8, 0.0, 0.0 ) ) );
    }
    ...
    {
        WGaussProcess p( m_tractID, m_tracts, m_emptyDTIDataSet );
        TS_ASSERT_DELTA( p.mean( WPosition_2( 0.0, 0.0, 0.0 ) ), p.m_maxLevel, 2 * wlimits::DBL_EPS );
    }

//    void testMeanFunctionOnSegmentButNotOnSamplePoint( void )
//    {
//        WGaussProcess p( m_tract, m_emptyDTIDataSet );
//        TS_ASSERT_DELTA( p.mean( WPosition_2( 0.4, 0.4, 0.0 ) ), p.m_maxLevel, wlimits::DBL_EPS );
//    }
    ...
    void testPartialOverlapWith10Points( void )
    {
        ...
        {
            tract.push_back( WPosition_2( static_cast< double >( i ), 0.0, 0.0 ) );
        }
        ...
        {
            tract.push_back( WPosition_2( 4.0, static_cast< double >( i ), 0.0 ) );
        }
        tracts->push_back( tract );
    ...
    void testPartialOverlapWith100Points( void )
    {
        ...
        {
            tract.push_back( WPosition_2( 49.0, static_cast< double >( i ), 0.0 ) );
        }
        tracts->push_back( tract );
        boost::shared_ptr< WDataSetFiberVector > fvDS( new WDataSetFiberVector( tracts ) );
...
protected:
    ...
    void setUp( void )
    {
        ...
        boost::shared_ptr< std::vector< WFiber > > tracts( new std::vector< WFiber > );
        WFiber tract0;
        tract0.push_back( WPosition_2( 0.0, 0.0, 0.0 ) );
        tract0.push_back( WPosition_2( 1.0, 1.0, 0.0 ) );
        tract0.push_back( WPosition_2( 1.0, 2.0, 0.0 ) );
        tract0.push_back( WPosition_2( 2.0, 2.0, 0.0 ) );
        tracts->push_back( tract0 );
        WFiber tract1;
        tract1.push_back( WPosition_2( 2.0 + 2.0 * sqrt( 8.0 ), 2.0, 0.0 ) );
        tract1.push_back( WPosition_2( 2.0 + 2.0 * sqrt( 8.0 ) + 1.0, 2.0, 0.0 ) );
