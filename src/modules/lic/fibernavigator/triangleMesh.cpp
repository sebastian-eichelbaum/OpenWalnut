    ...
    : m_grid( grid )
{
    ...
    {
        WPosition_2 pos = mesh->getVertexAsPosition( i );
...
{
    triangleColor[triNum] = WColor( r, g, b, a );
}
...
void TriangleMesh::setVertColor( const int vertNum, const WColor color)
{
    vertColors[vertNum] = color;
}

WColor TriangleMesh::getVertColor(const int vertNum)
{
...
int TriangleMesh::calcTriangleTensor(const int triNum)
{
    ...
    int x = (std::min)(static_cast< int >( m_grid->getNbCoordsX() ) - 1, (std::max)(0,(int)(p[0] + 0.5)));
    int y = (std::min)(static_cast< int >( m_grid->getNbCoordsY() ) - 1, (std::max)(0,(int)(p[1] + 0.5)));
    int z = (std::min)(static_cast< int >( m_grid->getNbCoordsZ() ) - 1, (std::max)(0,(int)(p[2] + 0.5)));
    return  x + y * m_grid->getNbCoordsX() + z * m_grid->getNbCoordsX() * m_grid->getNbCoordsY();
}
