      ...
      p0p2(3)
{
    m_mesh = mesh;
    isTensor = true;
    m_tensorField = new TensorField( vectors );
//    m_mesh->printInfo();
    ...
    try
    {
        ...
        cell_vectors.resize(nbTriangles);
        ...
        << "nbTriangles = " << (int) nbTriangles << std::endl;
#endif
        ...
        double length)
{
    ...
    try
    {
        ...
        {
#ifdef __DEBUG__
            std::cout << "left mesh in from first cell" << std::endl;
#endif
            return 1;
        }
        ...
        while (proceed())
        {
            ...
            else
            {
                ...
                m_mesh->getPosition( currPos, first);
            ...
            if (currLength + stepLength > length)
            {
                ...
                if (first.isValid() && second.isValid() && first == second)
                {
                    ...
                    m_mesh->getCellVerticesIndices(currCell, id);
...
void FStreamlineOnSurfaceEuler::setMaxNbTriangles(unsigned int maxNbTriangles)
{
    this->maxNbTriangles = maxNbTriangles;
}
        ...
        FIndex& nextCell, bool& posRot)
{
...
#endif
    m_mesh->getCellVerticesIndices(cellId, indices);
    for (positive i = 0; i < 3; ++i)
    {
        m_mesh->getPosition(vertices[i], indices[i]);
    }

    FTensor t1(cell_vectors[cellId.getIndex()]);


    FTensor t5 = m_tensorField->getTensorAtIndex(m_mesh->getTriangleTensor(cellId.getIndex()));
    m_mesh->setTriangleBlue(cellId.getIndex(), t5.norm());

    float fa = sqrt((std::min)((t5.norm() * 2.0), 1.0));


    Vector vt = m_mesh->getNormal(cellId.getIndex());
    FTensor t2(3, 1, true);
    t2.setValue(0, vt.x);
    t2.setValue(1, vt.y);
    t2.setValue(2, vt.z);
    float scalar = 1. - (acos(fabs(t1 * t2)) / 1.570796325);
    m_mesh->setTriangleGreen(cellId.getIndex(), scalar);

    m_mesh->setTriangleAlpha(cellId.getIndex(), 1.0 - (scalar * fa));
    ...
    if (onEdge)
    {
        ...
        if (u > -epsilon && u < 1. + epsilon)
        {
            ...
            m_mesh->getEdgeNeighbor(cellId, ids[1], neighs);
            if (neighs.size())
                nextCell = neighs[0];
            else
...
#endif
            // get neighboring cell
            m_mesh->getEdgeNeighbor(cellId, ids[2], neighs);
            if (neighs.size())
                nextCell = neighs[0];
            else
                nextCell.setToInvalid();
    ...
    // get neighboring cell
    m_mesh->getEdgeNeighbor(cellId, id0, neighs);
    if (neighs.size())
        nextCell = neighs[0];
    else
        nextCell.setToInvalid();
        ...
        FIndex& vertId1, FIndex& vertId2, double fwd, FArray& exit, FIndex& currCell, FIndex& nextCell)
{
    ...
    m_mesh->getNeighbors(vertId, neighs);

    // loop over neighbors
    for (positive i = 0; i < neighs.size(); i++)
    {
        m_mesh->getCellVerticesIndices(neighs[i], indices);
        for (positive j = 0; j < 3; j++)
        {
            m_mesh->getPosition(vertices[j], indices[j]);
        }
    ...
    if (next >= 0)
    {
        m_mesh->getCellVerticesIndices(neighs[next], indices);
        for (positive j = 0; j < 3; j++)
            m_mesh->getPosition(vertices[j], indices[j]);
        ...
        m_mesh-> getEdgeNeighbor(neighs[next], ids[1], neighs);
        if (neighs.size())
            nextCell = neighs[0];
        else
            nextCell.setToInvalid();
    ...
    m_mesh->getCellVerticesIndices(curr, indices);
    ...
    // 1st neighbor
    m_mesh-> getEdgeNeighbor(curr, k, neighs2);
    if (neighs2.size())
    {
        n1 = neighs2[0];
                ...
                * cell_vectors[curr.getIndex()]) < 0.)
        {
            ...
            m_mesh->getPosition(exit, indices[(k + 1) % 3]);
    ...
    // 2nd neighbor
    m_mesh-> getEdgeNeighbor(curr, (k + 2) % 3, neighs2);
    if (neighs2.size())
    {
        n2 = neighs2[0];
                ...
                * cell_vectors[curr.getIndex()]) < 0.)
        {
            ...
            m_mesh->getPosition(exit, indices[(k + 2) % 3]);
            // unable to determine next cell
            // use parameters to return cells sharing the edge we follow
            currCell = curr;
            nextCell = n1;
    ...
    if (n1.isValid())
    {
        m_mesh->getCellVerticesIndices(n1, indices);
        for (positive i = 0; i < 3; i++)
        {
            m_mesh->getPosition(vertices[i], indices[i]);
        }
        ...
        m_mesh->getEdgeNeighbor(n1, k, neighs2);
        ...
        m_mesh->getEdgeNeighbor(n1, (k + 2) % 3, neighs2);
        if (neighs2.size() && neighs2[0] != curr)
        {
            n3 = neighs2[0];
    ...
    if (n2.isValid())
    {
        m_mesh->getCellVerticesIndices(n2, indices);
        for (positive i = 0; i < 3; i++)
        {
            m_mesh->getPosition(vertices[i], indices[i]);
        }
        ...
        m_mesh-> getEdgeNeighbor(n2, (k + 1) % 3, neighs2);
        ...
        m_mesh->getEdgeNeighbor(n2, (k + 2) % 3, neighs2);
        if (neighs2.size() && neighs2[0] != curr)
        {
            n4 = neighs2[0];
