...
WTensorSym< 2, 3, double > WSurface::getCovarianceMatrix( std::vector< WVector3d_2 > points )
{
    WTensorSym< 2, 3, double > result;
    m_xAverage = m_yAverage = m_zAverage = 0;

    std::vector< WVector3d_2 >::iterator pointsIt;
    for( pointsIt = points.begin(); pointsIt != points.end(); ++pointsIt )
    {
        WVector3d_2 dmy = *pointsIt;
        m_xAverage += dmy[0];
        m_yAverage += dmy[1];
        m_zAverage += dmy[2];
    }
...
void WSurface::getSplineSurfaceDeBoorPoints( std::vector< WVector3d_2 > &givenPoints, std::vector< WVector3d_2 > &deBoorPoints, int numRows, int numCols ) // NOLINT
{
    ...
    for( int row = 0; row < numRows; ++row )
        for( int col = 0; col < numCols; ++col )
        {
            ...
            for( givenPointsIt = givenPoints.begin(); givenPointsIt != givenPoints.end(); ++givenPointsIt )
            {
                WVector3d_2 dmy1 = *givenPointsIt;
                WVector3d_2 dmyArray( dmy1[0], dmy1[1], dmy1[2] );
                dmyArray[1] = 0;
                WVector3d_2 thisPoint( x, 0, z );
            ...
            //</local shepard with franke-little-weights>
            deBoorPoints.push_back( WVector3d_2( x, y, z ) );
        }
    return;
}
...
{
    std::vector< WVector3d_2 > deBoorPoints;
    ...
    std::map< double, WVector3d_2 >::iterator sortedSystemIter = sortedEigenSystem.begin();
    ...
    for( pointsIt = m_supportPoints.begin(); pointsIt != m_supportPoints.end(); ++pointsIt )
    {
        ...
        WVector3d_2 dmy( ( *pointsIt )[0], ( *pointsIt )[1], ( *pointsIt )[2]  );

        WVector3d_2 result = transMatrix * dmy;
        ( *pointsIt )[0] = result[0];
        ( *pointsIt )[1] = result[1];
        ( *pointsIt )[2] = result[2];
    }
    ...
    for( std::vector< WVector3d_2 >::iterator posIt = m_splinePoints.begin(); posIt != m_splinePoints.end(); ++posIt )
    {
...
std::vector< WVector3d_2 > WSurface::getSplinePoints()
{
...
void WSurface::setSupportPoints( std::vector< WVector3d_2> supportPoints, bool forceUpdate )
{
